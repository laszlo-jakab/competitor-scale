...
# arrange plots
grid.arrange(
  scandal.fig$did$results$ln.CS, scandal.fig$did$results$ln.AS, scandal.fig$did$results$ff3, scandal.fig$did$results$ln.TL, scandal.fig$did$results$ln.T, scandal.fig$did$results$ln.L, nrow = 3)
...
```{r scandalSpillover, echo = FALSE, warning = FALSE, message = FALSE}
# Display DiD Table ------------------------------------------------------------
scandal.tab$did$results %>%
  kable(type = "html",
    caption = scandal.tab$did$title,
    align = c("l", rep("c", 7)),
    booktabs = TRUE,
    col.names = c("Dep. Var.:", "$\\ln(C.S.)$", "$\\ln(AS)$",
                  "$\\ln(TL^{-1/2})$", "$\\ln(L)$", "$\\ln(S)$",
                  "$\\ln(D)$", "$\\ln(C)$", "$\\ln(B)$")) %>%
  kable_styling(
    ...
    position = "center",
    font_size = 11) %>%
  group_rows("1 year window",
             1, dim(scandal.tab$did$sub.results$w1yr)[1], 
             label_row_css = "text-align: center;") %>%
  group_rows("2 year window",
             dim(scandal.tab$did$sub.results$w1yr)[1]+1, 
             dim(scandal.tab$did$results)[1], 
             label_row_css = "text-align: center;") %>%
  footnote(scandal.tab$did$caption)
...
grid.arrange(scandal.fig$sof$results$betas, scandal.fig$sof$results$pctiles, nrow = 1)
...
```{r scandalSpilloverIV, echo = FALSE}

# Display ScandalOutFlow Regressions ---------------------------------------

scandal.tab$sof$results %>%
  kable(type = "html",
  ...
  kable_styling(
    ...
    font_size = 11) %>%
  group_rows("1 year window",
             1, dim(scandal.tab$sof$sub.results$w1yr)[1], 
             label_row_css = "text-align: center;") %>%
  group_rows("2 year window",
             dim(scandal.tab$sof$sub.results$w1yr)[1]+1, 
             dim(scandal.tab$sof$results)[1], 
             label_row_css = "text-align: center;") %>%
  footnote(scandal.tab$sof$caption)
...
```{r scandalPerformance, echo = FALSE}

# Display Performance Regression Tables ---------------------------------------------

scandal.tab$perf$results %>%
  kable(type = "html",
  ...
  kable_styling(
    ...
    font_size = 11) %>%
  group_rows("1 year window",
             1, dim(scandal.tab$perf$sub.results$w1yr)[1], 
             label_row_css = "text-align: center;") %>%
  group_rows("2 year window",
             dim(scandal.tab$perf$sub.results$w1yr)[1]+1, 
             dim(scandal.tab$perf$results)[1], 
             label_row_css = "text-align: center;") %>%
  footnote(scandal.tab$perf$caption)
