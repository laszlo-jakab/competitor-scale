...
reg.performance$baseline$results %>%
  kable(type = "html", 
  ...
  kable_styling(
    ...
    position = "center",
    font_size = 11) %>%
  footnote(reg.performance$baseline$caption)
...
```{r roleOfLiquidity, echo = FALSE, warning = FALSE, message = FALSE}
# draw table ---------------------------------------------
reg.performance$liquidity$results %>%
  kable(type = "html",
  caption = reg.performance$liquidity$title,
  align = c("l", rep("c", 5)),
  booktabs = TRUE,
  col.names = c("$X=$", "$L$", "$S$", "$D$", "$C$", "$B$")) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover"), 
    position = "center",
    font_size = 11) %>%
  group_rows("Panel A: Fund Level Average Portfolio Liquidity", 
    1, dim(reg.performance$liquidity$sub.results$fund.level)[1], 
    label_row_css = "text-align: center;") %>%
  group_rows("Panel B: Real Time Portfolio Liquidity", 
    dim(reg.performance$liquidity$sub.results$fund.level)[1]+1, 
    dim(reg.performance$liquidity$results)[1], 
    label_row_css = "text-align: center;") %>%
  footnote(reg.performance$liquidity$caption)
