...
com <- function(nc, txt) {
  c(TCap(nc, tab.scandal$did$caption),
...
$$
flow_{i,t}=\alpha_i + \alpha_t + \gamma Post_{i,t} + \sum_{\tau = 1}^{12} R_{i, t-\tau} +\varepsilon_{i,t},
$$
where $Post_{i,t}$ is an indicator for post news date for scandal funds. I find $\gamma=$`r formatC(as.numeric(gsub("\\*+", "", tab.flow$scandal.treat$results[1, 4]))*12, digits = 1, format = "f")`\% per year, with t-statistic of `r formatC(as.numeric(gsub("[\\(\\)]", "", tab.flow$scandal.treat$results[2, 4])), digits = 1, format = "f")`.] 
...
```{r exposureDID, echo = FALSE, dpi = 300, fig.width = 9, fig.height = 11, fig.align = 'center', fig.cap = paste(fig.scandal$did$title, fig.scandal$did$caption), fig.scap = fig.scandal$did$title}
# DiD Plots --------------------------------------------------------------------

# arrange plots
grid.arrange(
  fig.scandal$did$results$ln.CS, fig.scandal$did$results$ln.AS, 
  fig.scandal$did$results$ff3, fig.scandal$did$results$ln.TL, 
  fig.scandal$did$results$ln.T, fig.scandal$did$results$ln.L, 
  nrow = 3)
# -------------------------------------------------------------------------------
```
...
# label columns
setnames(tab.scandal$did$results, 
...
Table \@ref(tab:scandalSpillover) presents results. The one (two) year window estimate implies a `r formatC(abs(as.numeric(gsub("\\*", "", tab.scandal$did$sub.results$w1yr[1, ln.CS])))*100, digits = 1, format = "f")`\% (`r formatC(abs(as.numeric(gsub("\\*", "", tab.scandal$did$sub.results$w2yr[1, ln.CS])))*100, digits = 1, format = "f")`\%) post-scandal reduction in $CompetitorSize$ for untainted funds at the 75$^{\text{th}}$ percentile of $ScandalExposure$ relative to untainted funds at the 25$^{\text{th}}$ percentile of $ScandalExposure$. 

**The same difference in $ScandalExposure$ is associated with a `r formatC(abs(as.numeric(gsub("\\*", "", tab.scandal$did$sub.results$w1yr[1, ln.AS])))*100, digits = 1, format = "f")`\% (`r formatC(abs(as.numeric(gsub("\\*", "", tab.scandal$did$sub.results$w2yr[1, ln.CS])))*100, digits = 1, format = "f")`\%) relative increase in active share, and a `r formatC(as.numeric(gsub("\\*", "", tab.scandal$did$sub.results$w1yr[1, ln.TL]))*100, digits = 1, format = "f")`\% (`r formatC(as.numeric(gsub("\\*", "", tab.scandal$did$sub.results$w2yr[1, ln.TL]))*100, digits = 1, format = "f")`\%) change in the turnover to portfolio liquidity ratio. The coefficients also imply a relative decline in portfolio liquidity and its components for high exposure funds. These findings are consistent with high exposure funds increasing activeness in response to softened competition.** FIX THIS
...
# label columns
setnames(tab.scandal$sof$results, 
  c("Dep. Var.:", "$\\ln(C.S.)$", "$\\ln(AS)$",
    "$\\ln(TL^{-1/2})$", "$\\ln(L)$", "$\\ln(S)$",
    "$\\ln(D)$", "$\\ln(C)$", "$\\ln(B)$"))
...
# fix first row
setnames(tab.scandal$perf$results, "V1", "")
